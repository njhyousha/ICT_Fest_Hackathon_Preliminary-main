...
def get_report(org_id: int, frm: str, to: str):
    report = _report_cache.get((org_id, frm, to))
    return report.copy() if report is not None else None


def set_report(org_id: int, frm: str, to: str, value: dict) -> None:
    _report_cache[(org_id, frm, to)] = value.copy()


def invalidate_report(org_id: int) -> None:
    for key in [k for k in list(_report_cache.keys()) if k[0] == org_id]:
        _report_cache.pop(key, None)


def get_availability(room_id: int, date: str):
    availability = _availability_cache.get((room_id, date))
    return availability.copy() if availability is not None else None


def set_availability(room_id: int, date: str, value: dict) -> None:
    _availability_cache[(room_id, date)] = value.copy()
